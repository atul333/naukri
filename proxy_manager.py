...
class ProxyManager:
    ...
    def __init__(self, proxies_file=None):
        ...
        self.proxies_file = proxies_file or os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "proxies.txt"
        )
        self.webshare_proxies = self._load_proxies_from_file()

    def _load_proxies_from_file(self):
        """Load proxy list from proxies.txt (one proxy URL per line)."""
        if os.path.exists(self.proxies_file):
            try:
                ...
                if proxies:
                    logger.info(f"📋 Loaded {len(proxies)} proxies from {self.proxies_file}")
                    return proxies
                else:
                    logger.warning(f"⚠️ {self.proxies_file} is empty, using fallback hardcoded proxies")
            except Exception as e:
                logger.warning(f"⚠️ Could not read {self.proxies_file}: {e}")

        # Fallback hardcoded proxies (update proxies.txt for easier management)
        logger.warning("⚠️ proxies.txt not found, falling back to hardcoded proxy list")
        return [
